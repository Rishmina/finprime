    ...
    <div className="xl:ml-12 xl:mr-10 lg:ml-10 lg:mr-8 md:ml-8 md:mr-6 ml-6 mr-4 max-w-screen md:my-20">
      <div className="flex flex-col md:flex-row items-center md:items-start gap-8">
        ...
        <div className="w-full md:w-3/4">
          ...
          <div className="flex flex-row flex-wrap justify-center md:justify-start items-start gap-x-14 gap-y-4 md:gap-y-12 mb-20">
            {/* CEO Image Card */}
            <div className="flex flex-col items-center text-center flex-shrink-0" style={{ width: '260px', height: '340px' }}>
              <div className="relative w-full h-full pb-12 md:pb-0 mb-8 md:mb-0">
                <img
                  src={ceoCard.image}
                  alt={ceoCard.alt}
            ...
            {otherCards.slice(0, 3).map((card, idx) => {
              const isSpecial = card.name === 'Mohamed Oun' || card.name === 'Mohamed Althaf' || card.name === 'C.A Zain Rabab';
              const isMohamedOun = card.name === 'Mohamed Oun';
              const isMohamedAlthaf = card.name === 'Mohamed Althaf';
              const isZainRabab = card.name === 'C.A Zain Rabab';
              return (
                <div key={card.number} className={`flex flex-col items-center text-center flex-shrink-0 ${isMohamedOun ? 'mt-16 md:mt-8' : isMohamedAlthaf ? 'mt-8' : isZainRabab ? 'mt-24 md:mt-8' : 'mt-8'} ${isMohamedAlthaf ? 'h-[240px] justify-center' : isSpecial ? 'h-[240px] justify-center' : 'h-auto'} md:h-auto`} style={{ width: '180px' }}>
                  <div className={`relative w-full ${isMohamedAlthaf ? 'h-[180px] flex items-center justify-center' : isZainRabab ? 'h-[180px] flex items-center justify-center mt-6 md:mt-0' : isSpecial ? 'h-[180px] flex items-center justify-center' : 'h-[220px]'} mb-0`}>
                    <img
                      src={card.image}
                      alt={card.alt}
                      style={{ objectPosition: isMohamedAlthaf ? 'center 20%' : isSpecial ? 'center 20%' : 'center top', width: '180px', height: isMohamedAlthaf ? '180px' : isSpecial ? '180px' : '220px', objectFit: 'cover', boxShadow: '0 4px 16px rgba(0,0,0,0.15)' }}
                    />
                  ...
                  <div className={`${isZainRabab ? 'mt-20 md:mt-6' : isMohamedAlthaf ? 'mt-4' : isSpecial ? 'mt-4' : 'mt-6'}`}>
                    <h4
          ...
          <div className="flex flex-wrap justify-center md:justify-start gap-x-16 gap-y-12 md:-ml-48">
            {otherCards.slice(3).map(card => (
              <div key={card.number} className="flex flex-col items-center text-center flex-shrink-0">
                <div className="relative w-[180px] h-[220px] pb-12 md:pb-0 mb-8 md:mb-0">
                  <img
                    src={card.image}
                    alt={card.alt}
